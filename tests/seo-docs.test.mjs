...
assertCurlBearerOffArgvPattern(api, 'API reference', 33);
...
}
{
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(withoutConfigStdin, 'mutated-api', 33),
    /config/i,
    'assertCurlBearerOffArgvPattern must reject curl snippet without stdin config',
  );
}
{
  // 靶向突变 1：删掉整条 printf 行（token 引用随之而去）——计数 33→32 必须红。
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(singleBlockAuthLoss, 'single-block-loss', 33),
    /found \d+/,
...
}
{
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(catBypass, 'cat-bypass', 33),
...
}
{
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(onwardPipe, 'onward-pipe', 33),
    /token variable|found \d+/,
...
}
{
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(andEcho, 'and-echo', 33),
    /token variable|found \d+/,
...
}
{
  ...
  assert.throws(
    () => assertCurlBearerOffArgvPattern(commentedProducer, 'commented-producer', 33),
    /found \d+/,
...
// （含 ${VAR} 形态）；按文件钉数量 1/2/33。假绿反例（各轮闸面实证）：printf 灌
